...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    if rank in [-1, 0]:
        ...
        if not opt.resume:
            ...
            if not opt.noautoanchor:
                check_anchors(detect_dataset, model=model, thr=hyp['anchor_t'], imgsz=imgsz, autoAnchor=True)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (detect_imgs, detect_targets, detect_paths, _) in detect_phbar:  # batch -------------------------------------------------------------
            ...
            # Forward
            if faceQt_targets.size(0) > 0 and faceQt_compute_loss.check_targets(faceQt_targets.to(device)):
                with amp.autocast(enabled=cuda):
                    faceQt_pred_ = []
