...
from models.detectwithfacekp_yolo_roi import Model
...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    detect_dataloader, detect_dataset = detect_datasets.create_dataloader(detect_train_path, imgsz, batch_size, gs, single_cls,
                                            ...
                                            world_size=opt.world_size, workers=opt.workers,
                                            image_weights=opt.image_weights, quad=opt.quad, prefix=colorstr('train: '))
    faceQt_dataloader, faceQt_dataset = faceqt_datasets.create_dataloader(faceQt_train_path, imgsz, batch_size, gs,
                                                                          single_cls,
    ...
    if rank in [-1, 0]:
        ...
        if not opt.resume:
            ...
            if not opt.noautoanchor:
                check_anchors(detect_dataset, model=model, thr=hyp['anchor_t'], imgsz=imgsz)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (detect_imgs, detect_targets, detect_paths, _) in detect_phbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                faceQt_pred_ = []
                faceQt_pred = model(faceQt_imgs)# forward
                for predItem in faceQt_pred:
                    faceQt = torch.cat((predItem[:,:,:,:,0:5], predItem[:,:,:,:,15:16]), dim=4)
                    faceQt_pred_.append(faceQt)
                faceQtloss, faceQtloss_items = faceQt_compute_loss(faceQt_pred_, faceQt_targets.to(device))
                if rank != -1:
                    faceQtloss *= opt.world_size  # gradient averaged between devices in DDP mode
                if opt.quad:
                    faceQtloss *= 4.
                loss = detectloss
            # Backward
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default='/home/chase/shy/hyolov5/data/detectWithfaceKp.yaml', help='dataset.yaml path')
