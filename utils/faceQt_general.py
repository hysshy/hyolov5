...
def xywhn2xyxy(x, w=640, h=640, padw=0, padh=0):
    ...
    y[:, 12] = np.array(x[:, 12] > 0, dtype=np.int32) * (w * x[:, 12] + padw) + (np.array(x[:, 12] > 0, dtype=np.int32) - 1)
    y[:, 13] = np.array(x[:, 13] > 0, dtype=np.int32) * (h * x[:, 13] + padh) + (np.array(x[:, 13] > 0, dtype=np.int32) - 1)
    return y
...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if multi_label:
            ...
            x = torch.cat((box, conf, x[:, -1:], j.float()), 1)[conf.view(-1) > conf_thres]
